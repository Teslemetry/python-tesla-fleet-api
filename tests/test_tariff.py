...
``tests/fixtures/tariff_moorinya_v2.json`` is a live-shaped Tariff V2 capture
(48 half-hour buy/sell periods, one covering the whole year) modeled on the
design report's live Moorinya capture, including its two real anomalies:
...
FIXTURE_PATH = Path(__file__).parent / "fixtures" / "tariff_moorinya_v2.json"
TZ = ZoneInfo("Australia/Brisbane")


def _load_moorinya() -> dict:
    return json.loads(FIXTURE_PATH.read_text())


def _moorinya_tariff() -> dict:
    return copy.deepcopy(unwrap_tariff_v2(_load_moorinya()))


class UnwrapTariffV2Tests(TestCase):
    def test_site_info_envelope(self):
        response = _load_moorinya()
...
class MoorinyaFixtureTests(TestCase):
    """Cover the live-shaped fixture's own anomalies."""

    def setUp(self):
        self.tariff = _moorinya_tariff()
